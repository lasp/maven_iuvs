...
import numpy as np
from maven_iuvs.geometry import get_pixel_vec_mso, pixelcorner_avg
...
def quicklook_apoapse(fig=None,orbno=None, observations=None,
    ...
    for idx,obs in enumerate(apoapse_obs):
        ...
        # to do this, project out to the plane that intersects the limb, not the plane that passes through the center of Mars.
        # this plane lives at distance d0 = r_sc - R_mars*(R_mars/r_sc) = (r_sc**2 - R_Mars**2)/r_sc along the sc/Mars direction
        # d0/cos(theta), where theta is the angle off nadir
    ...
    direction_change=(start_along_mirror[1:]-end_along_mirror[:-1]+0.05)
    direction_change*=np.sign(np.mean(end_along_mirror-start_along_mirror))
