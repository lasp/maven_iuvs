...
fuv_lsf_seven_segment=readsav(os.path.join(os.path.split(getsourcefile(iuvs))[0],'anc','mvn_iuv_psf_fuv_2017APR24.sav'))
...
for k in fuv_lsf_seven_segment.keys():
    ...
    waves=5*(np.arange(len(fuv_lsf_seven_segment[k]))/len(fuv_lsf_seven_segment[k])-0.5)
...
seven_segment_flatfield=[0.976, 1.018, 1.018,  1.022, 1.004, 0.99, 0.953] #from sonal, derived from Lyman alpha (so technicallly only valid there)
...
def fit_line(myfits,l0,calibrate=True,flatfield_correct=True,plotspec=False,plotdev=False):
    ...
    nspa=filedims[1]
    if nspa != 7:
        raise Exception("fit_line requires the FITS file to have seven spatial elements, like a periapsis or side segment file.")
    ...
    for iint in range(nint):
        for ispa in range(nspa):
            ...
            if plotspec:
                ...
                ax[iint][ispa].step(fitwaves,this_spatial_element_lsf(fitwaves,*fit[0]))
                ax[iint][ispa].set_ylim(0,1.05*lineDNmax)
                ax[iint][ispa].get_shared_y_axes().join(ax[0][0], ax[iint][ispa])

            if plotdev:
                ##plot deviations
                ax[iint][ispa].step(fitwaves,(fitDN-this_spatial_element_lsf(fitwaves,*fit[0]))/np.sum(fitDN))
...
def get_line_calibration(myfits,
    ...
    if myfits['Observation'].data['DUTY_CYCLE'][0] != 1.0:
        raise Exception("Duty cycle != 1.0 , line calibration routine cannot handle this")
    ...
    if xuv=='FUV':
        ...
        line_effective_area = np.interp(line_center,
                                        effective_area['waveg']/10.-wavelength_shift,
        ...
        wavelength_shift = 7.0 # shift MUV calibration by 7 nm redward
                               ...
                               # derived calibration
        line_effective_area = np.interp(line_center,
                                        effective_area['wavef']/10.-wavelength_shift,
                                        # /10 here Angstroms -> nm
    ...
    calfactor = gain * inttime * kr * line_effective_area * bin_omega # DN/photon * s * photon/kR * cm2 * sr 
