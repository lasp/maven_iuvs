...
# This creates a single quicklook independent of geometry for 8 swaths
...
from quicklook_constants import FlatField, HEIGHT, NSWATHS, POLAR, WIDTH, PREMADE_COLORING, \
    CYLINDRICAL, SZA_CORRECTION
from plotting import cylindrical_map, plot_quicklook, polar_plot, plot_topographic_quicklook
...
def quicklook(orbit):
    ...
    for i in range(n_files):
        ...
        for j in range(n_integrations):
            try:
                # Flat-field correct the data
                primary[j, :, red_index] /= FlatField[:, red_index]
                primary[j, :, green_index] /= FlatField[:, green_index]
                primary[j, :, blue_index] /= FlatField[:, blue_index]
            except:
                pass

        # Now pull things out of the data starting with the DNs for color
        red_data = primary[:, :, red_index]
        green_data = primary[:, :, green_index]
        blue_data = primary[:, :, blue_index]
        ...
        if geometry:
            ...
            if SZA_CORRECTION:
                ...
                # If I'm not SZA correcting, do this
    ...
    with plt.style.context(('dark_background')):   # Force the quicklook to be black but not any other potential plots
        if geometry:
            # Make my plot
            plot_quicklook(data_array, local_time_array, orbit_number, expanded_date, solar_longitude, version,
                           block, local_time_inset=True)
            # Make Nick's plot
            plot_quicklook(data_array, local_time_array, orbit_number, expanded_date, solar_longitude, version,
                           block, local_time_inset=False)
        else:
            plot_quicklook(data_array, 'no_local_times', orbit_number, expanded_date, solar_longitude, version,
                           block)
    ...
    if geometry:
        ...
        with plt.style.context(('dark_background')):
            # Make my plot
            plot_topographic_quicklook(data_array, local_time_array, map_array, orbit_number, expanded_date,
                                       solar_longitude, version, block, crap_local_times=False)
            # Make Nick's plot
            plot_topographic_quicklook(data_array, local_time_array, map_array, orbit_number, expanded_date,
                                       solar_longitude, version, block, crap_local_times=True)
